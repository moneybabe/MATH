...
mae_training = mean(abs(cmort[first_half] - training_fitted)[-1:-4])
...
for (k in lags) {
  lagged_data[[paste0("lagged_part_", k)]] = dplyr::lag(part_vec, k)
  lagged_data[[paste0("lagged_tempr_", k)]] = dplyr::lag(tempr_vec, k)
}

# Combine lagged data into a data frame
lagged_df = as.data.frame(cbind(lagged_data))
...
training_fitted = predict(reg_first_half, newdata = lagged_df[first_half,])

mae_training = mean(abs(cmort[first_half] - training_fitted)[-1:-max(lags)])

# ... [plotting code]


# Now, for the initial training set
reg_first_half = lm(cmort_vec ~ mat, subset = first_half)

training_fitted = predict(reg_first_half, newdata = as.data.frame(mat[first_half, ]))
